...
if __name__ == "__main__":
    ...
    interface = IcechunkInterface(dataset_key)
    ...
    if os.path.exists(pkl_name):
        ...
        nc_file_info = interface.init_nc_file_data()
    ...
    drop_vars = interface.dataset_config.get("drop_vars")

    repo_timestamps = interface.get_repo_timestamps()
    ...
    for timestamp in timestamps:
        ...
        while True:
            ...
            try:
                session = interface.repo.writable_session("main")
                with ProcessPoolExecutor() as executor:
                    fork = session.fork()
                    futures = [
                        executor.submit(
                            interface.append_timestamp,
                            nc_file=f,
                            drop_vars=drop_vars,
